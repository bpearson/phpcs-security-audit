...
class Security_Sniffs_CVE_20134113Sniff implements \PHP_CodeSniffer\Sniffs\Sniff {
	...
	public function process(\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr) {
		...
		if ($tokens[$stackPtr]['content'] == 'xml_parse_into_struct') {
			...
			$s = $stackPtr + 1; // to skip the parenthesis opener '('
						$s = $phpcsFile->findNext(array_merge(\PHP_CodeSniffer\Util\Tokens::$emptyTokens, Security_Sniffs_Utils::$staticTokens), $s + 1, $closer, true);
			if ($s) {
